...
window.onload = async function () {
  ...
  try {
    [categoriesCache, productsCache, paymentMethodsCache, activePromosCache] =
      await Promise.all([
        apiGetCategories(),
        apiGetProducts(),
...
// ── LOGOUT ──
function logout() {
  if (cart.length > 0 && !confirm("Bạn có sản phẩm trong hóa đơn. Đăng xuất?"))
    return;
...
function renderCategoryTabs() {
  ...
  let html =
    '<button class="cat-tab active" onclick="filterByCategory(\'ALL\', this)">Tất cả</button>';
...
function applyFilters() {
  ...
  if (query) {
    filtered = filtered.filter(
      (sp) =>
        sp.TenSP.toLowerCase().includes(query) ||
        sp.MaSP.toLowerCase().includes(query),
    );
  }
...
function calculateTotal() {
  ...
  if (pointsDiscount > 0) {
    ...
    document.getElementById("points-discount").innerText =
      "-" + fmtVND(pointsDiscount);
  } else {
