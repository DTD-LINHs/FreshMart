...
async function renderPaymentMethods() {
    try {
        ...
        container.innerHTML = methods.map(pt => `
...
function confirmPayment() {
    ...
    if (!selectedMaPT) return showToast('Vui lòng chọn phương thức thanh toán!', 'warning');
    if (!currentCustomer) return showToast('Vui lòng tìm khách hàng trước!', 'warning');
...
async function showReceipt() {
    ...
    document.getElementById('rec-method').innerText = payment.TenPT + ' (' + payment.MaPT + ')';
...
async function finishOrder() {
    ...
    try {
        ...
        showToast('Hoàn tất! Hóa đơn ' + result.MaHD + '. Điểm tích lũy: ' + result.points_earned, 'success');
